...
if (NOT MKL_ROOT_DIR)
    if (MKL_FIND_REQUIRED)
		...
		message(STATUS "Building without MKL")
    ...
    # first the libs
    set(MKL_LINK_TOOL_ARGUMENTS "-libs")
    ...
    if (MKL_INCLUDE_DIR MATCHES "Composer.*2013")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=ComposerXE2013")
    elseif (MKL_INCLUDE_DIR MATCHES "Composer.*2011")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=ComposerXE2011")
    elseif (MKL_INCLUDE_DIR MATCHES "10.3")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=10.3")
    elseif(MKL_INCLUDE_DIR MATCHES "2013") # version 11 ...
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=11.1")
    elseif(MKL_INCLUDE_DIR MATCHES "2015")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=11.2")
    elseif(MKL_INCLUDE_DIR MATCHES "2016")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=11.3")
    elseif(MKL_INCLUDE_DIR MATCHES "2017")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=11.3")
    elseif(MKL_INCLUDE_DIR MATCHES "2018")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=11.3")
    elseif (MKL_INCLUDE_DIR MATCHES "10")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=10.2")
    else()
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--mkl=11.3")
    endif()

    if (CMAKE_CXX_COMPILER_ID STREQUAL "Clang")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--compiler=clang")
    elseif(CMAKE_CXX_COMPILER_ID STREQUAL "Intel")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--compiler=intel_c")
    elseif(CMAKE_CXX_COMPILER_ID STREQUAL "MSVC")
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--compiler=ms_c")
    else()
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--compiler=gnu_c")
    endif()

	# Set the TBB search library path search suffix based on the version of VC
	if(WINDOWS_STORE)
		set(MKL_TBB_LIB_PATH_SUFFIX "vc11_ui")
	elseif(MSVC14)
		set(MKL_TBB_LIB_PATH_SUFFIX "vc14")
	elseif(MSVC12)
		set(MKL_TBB_LIB_PATH_SUFFIX "vc12")
	elseif(MSVC11)
		set(MKL_TBB_LIB_PATH_SUFFIX "vc11")
	elseif(MSVC10)
		set(MKL_TBB_LIB_PATH_SUFFIX "vc10")
	endif()

    if (APPLE)
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--os=mac")
    elseif(WIN32)
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--os=win")
    else()
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--os=lnx")
    endif()

    set(MKL_LIB_DIR)
    if (${CMAKE_SIZEOF_VOID_P} EQUAL 8)
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--arch=intel64")
        set(MKL_LIB_DIR "intel64")
    else()
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--arch=ia-32")
        set(MKL_LIB_DIR "ia32")
    endif()

    if (MKL_USE_sdl)
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--linking=sdl")
    else()
        if (BLA_STATIC)
            list(APPEND MKL_LINK_TOOL_ARGUMENTS "--linking=static")
        else()
            list(APPEND MKL_LINK_TOOL_ARGUMENTS "--linking=dynamic")
        endif()
    endif()

    if (MKL_USE_parallel)
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--parallel=yes")
    else()
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--parallel=no")
    endif()

    if (FORCE_BUILD_32BITS)
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--interface=cdecl")
        set(MKL_USE_interface "cdecl" CACHE STRING "disabled by FORCE_BUILD_32BITS" FORCE)
    else()
        list(APPEND MKL_LINK_TOOL_ARGUMENTS "--interface=${MKL_USE_interface}")
    endif()

    if (MKL_USE_parallel)
		if (MKL_USE_parallel_library)
			list(APPEND MKL_LINK_TOOL_ARGUMENTS "--threading-library=${MKL_USE_parallel_library}")
			list(APPEND MKL_LINK_TOOL_ARGUMENTS "--openmp=${MKL_USE_parallel_library}")
		else()
			if (UNIX AND NOT APPLE)
				list(APPEND MKL_LINK_TOOL_ARGUMENTS "--openmp=gomp")
			else()
				list(APPEND MKL_LINK_TOOL_ARGUMENTS "--threading-library=iomp5")
				list(APPEND MKL_LINK_TOOL_ARGUMENTS "--openmp=iomp5")
			endif()
		endif()
    endif()

    execute_process(COMMAND ${MKL_LINK_TOOL} ${MKL_LINK_TOOL_ARGUMENTS}
    ...
    if (NOT ${COMMAND_WORKED} EQUAL 0)
        message(FATAL_ERROR "Cannot find the MKL libraries correctly. Please check your MKL input variables and mkl_link_tool. The command executed was:\n ${MKL_LINK_TOOL_ARGUMENTS}.")
    endif()

    set(MKL_LIBRARY_DIR)

    if (WIN32)
        set(MKL_LIBRARY_DIR "${MKL_ROOT_DIR}/lib/${MKL_LIB_DIR}/" "${MKL_ROOT_DIR}/../compiler/lib/${MKL_LIB_DIR}" "${MKL_ROOT_DIR}/../tbb/lib/${MKL_LIB_DIR}/${MKL_TBB_LIB_PATH_SUFFIX}")
    ...
    # now definitions
    string(REPLACE "-libs" "-opts" MKL_LINK_TOOL_ARGUMENTS "${MKL_LINK_TOOL_ARGUMENTS}")
    execute_process(COMMAND ${MKL_LINK_TOOL} ${MKL_LINK_TOOL_ARGUMENTS} OUTPUT_VARIABLE RESULT_OPTS TIMEOUT 2 ERROR_QUIET)
    string(REGEX MATCHALL "[-/]D[^\ ]*" MKL_DEFINITIONS ${RESULT_OPTS})

    if (CMAKE_FIND_DEBUG_MODE)
        message(STATUS "Exectuted command: ${MKL_LINK_TOOL}")
        message(STATUS "Found MKL_LIBRARIES: ${MKL_LIBRARIES} ")
        message(STATUS "Found MKL_DEFINITIONS: ${MKL_DEFINITIONS} ")
