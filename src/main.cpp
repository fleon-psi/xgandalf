...
int main()
{
    ...
    ExperimentSettings experimentSettings(coffset_m, clen_mm, beamEenergy_eV, divergenceAngle_deg, nonMonochromaticity,
            pixelLength_m, detectorRadius_pixel, sampleReciprocalLattice_1A);

    Indexer indexer(experimentSettings);

    Matrix2Xf detectorPeaks_m;
    loadEigenMatrixFromDisk(detectorPeaks_m, "workfolder/detectorPeaks_m");
